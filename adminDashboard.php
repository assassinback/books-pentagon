...
<head>
    ...
    <title>BooksPentagon  -  Admin Dashboard</title>
...
?>
<div class="rb1">
<form action="./searchUserAdmin.php" method="post">
    <input type="submit" value="Search a user" name="search">
</form>
<form method="post">
...
<?php
    ...
    if(isset($_COOKIE['username'])&&isset($_COOKIE['password']))
    {
        ...
        if($rs=mysqli_query($con,$query))
        {
            ...
            echo '<th>Email</th>';
            echo '<th>Bokos Published</th>';
            
            echo '<th>Delete</th>';
            echo '<th>View published Papers</th>';
            
            echo '<th>View Payments</th>';
            while($row=mysqli_fetch_array($rs))
            {
                ...
                <td border="1"><form method="POST" action="ViewPayments.php"><input type="submit" value="View Payments" name="pub">
                <input type="hidden" name="userid" value="'.$row["id"].'"></form></td>
                </tr>
                ';    
            }
    ...
    if(isset($_COOKIE['username'])&&isset($_COOKIE['password'])&&isset($_POST['delete']))
        ...
        if(isset($_COOKIE['username'])&&isset($_COOKIE['password'])&&isset($_POST['published']))
        {
            setcookie('userid',$_POST["userid"]);
            header('Location:viewPublished.php');
            ob_end_flush();
        }
        if(isset($_COOKIE['username'])&&isset($_COOKIE['password'])&&isset($_POST['waiting']))
        {
            setcookie('userid',$_POST["userid"]);
            header('Location:viewWaiting.php');
            ob_end_flush();
        }
?>
    <div class="rb1"><button id="hidebtn">Show UserInfo</button></div>
    <script>
